...
class BaseRepository {
    ...
    func callApi<T>(url: String, method: String, parameters: Dictionary<String, Any>, onComplete: @escaping (ApiResponse<T>) -> ()) {
        if (method == "GET") {
            self.callApiGetCore(onComplete: onComplete, emitter: nil, url: url, method: method, parameters: parameters)
        } else if (method == "POST") {
            self.callApiPostCore(onComplete: onComplete, emitter: nil, url: url, method: method, parameters: parameters)
        }
    }
    ...
    func callApi<T>(url: String, method: String, parameters: Dictionary<String, Any>) -> Observable<ApiResponse<T>> {
        return Observable<ApiResponse<T>>.create { (emitter) -> Disposable in
            ...
            if (method == "GET") {
                self.callApiGetCore(onComplete: nil, emitter: emitter, url: url, method: method, parameters: parameters)
            } else if (method == "POST") {
                self.callApiPostCore(onComplete: nil, emitter: emitter, url: url, method: method, parameters: parameters)
            }
            return Disposables.create()
        }
    }
    
    func callApiPostCore<T>(onComplete: ((ApiResponse<T>) -> ())?, emitter: (AnyObserver<ApiResponse<T>>)?, url: String, method: String, parameters: Dictionary<String, Any>) {
        ...
        AF.request(url, method: httpMethod, parameters: parameters, encoding: JSONEncoding.default, headers: header).responseJSON(completionHandler: {response in
            ...
            case .success(let data):
                ...
                let resultStatus: Int = jsonData["code"]?.int ?? ApiConst.STATUS_SUCCESS
    ...
    func callApiGetCore<T>(onComplete: ((ApiResponse<T>) -> ())?, emitter: (AnyObserver<ApiResponse<T>>)?, url: String, method: String, parameters: Dictionary<String, Any>) {
        var httpMethod: HTTPMethod = .get
